...
{
    /// <summary>
    /// 一鍵建置工具：產生所有 UI View prefabs（Resources/UIViews）與遊戲場景（Assets/Scenes/Game.unity）。
    /// 版面以 3840x2160（4K）為設計解析度。
    /// 可重複執行（會覆寫既有產物）。美術調整建議直接改 prefab；重跑本工具會還原為基礎版面。
    /// </summary>
    public static class GameUiBuilder
    {
        private const string PREFAB_FOLDER = "Assets/ProjectII/Resources/UIViews";
        private const string SCENE_PATH = "Assets/Scenes/Game.unity";
        ...
        [MenuItem("ProjectII/Build UI Prefabs And Scene")]
        public static void BuildAll()
        {
            ...
            Debug.Log("[GameUiBuilder] UI prefabs 與 Game 場景建置完成（3840x2160）。");
        }
        ...
        private static void BuildGameScene()
        {
            ...
            GameLauncher launcher = launcherObject.AddComponent<GameLauncher>();
        ...
        private static GameObject InstantiateDialogueView(Transform canvasTransform)
        {
            ...
            if (prefab == null)
            {
                Debug.LogError($"[GameUiBuilder] 找不到 DialogueView prefab：{DIALOGUE_VIEW_PREFAB_PATH}，請手動拖入場景並指定給 GameLauncher。");
                return null;
            }
        ...
        private static void SetFloat(Component target, string fieldName, float value)
        {
            ...
            if (property == null)
            {
                Debug.LogError($"[GameUiBuilder] {target.GetType().Name} 找不到欄位 {fieldName}");
                return;
            }
